export const SETUP_PROMPT_ZH = [
  '把 https://github.com/aa2246740/dsh-external-plugin-devkit 装进我本机的 DeepSeek Harness：放到 <harness>/tools/dshx，在 ~/.local/bin 安装 dshx 用户 launcher（不要改 Harness package.json），把仓库里的 skill/dshx 装到我机器上已有的 Agent skill 目录，写入 ~/.config/dshx/harness，然后跑 dshx which 和 dshx doctor。',
  '多个 Harness checkout 就停下来问，不要猜。不要启动或杀掉 dsh。不要写死别人的机器路径。',
].join('')

export const SETUP_PROMPT_EN = [
  'Install https://github.com/aa2246740/dsh-external-plugin-devkit into my local DeepSeek Harness: place it at <harness>/tools/dshx, install the user launcher under ~/.local/bin without editing Harness package.json, install skill/dshx into every Agent skill home that already exists on this machine, write ~/.config/dshx/harness, then run dshx which and dshx doctor.',
