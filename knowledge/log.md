...
* **Codex `$skill` example**: added `examples/codex-skill` (host `$name` pre-step, same `<skill_content>` as official `/name`) plus [codex-skill](/contracts/codex-skill.md). Codex does **not** invoke skills with `/`; `$` composer picker still needs the optional TriggerChar core patch.
