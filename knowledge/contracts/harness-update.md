...
- 只接受官方 `deepseek-ai/deepseek-harness` origin 和 `dsh-v*` release tag。省略 `--target` 时目标是工作台钉 `dsh-v0.1.7-rc.2`（SHA `477b4f420553e8a52c2fbccc464d7561b239c443`），不是 origin 上版本号更高的 alpha。显式 `--target dsh-v0.1.7-rc.2` 解析同一枚 tag。
- 新脚手架写入的 `@deepseek-ai/dsh` / `@deepseek-ai/dsh-*` peer 与 dev 范围是 `>=0.1.7-rc.1 <0.1.8`。Host 在导入插件前用这个范围对照运行时版本；该范围接受 `0.1.7-rc.2`，拒绝 `0.1.7` alpha。`^0.1.5-rc.3` 接受不了 `0.1.7-rc.1` 或 `0.1.7-rc.2`。
