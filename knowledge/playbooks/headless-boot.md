...
description: 无 Web UI 的一次性任务。用 dshx start headless --task，或 verify --profile headless。不是新会话急救。
...
pnpm dshx verify hello --profile headless
```
...
`verify --profile headless` 默认任务是 `reply with the single word pong and stop`。它断言 marker（`apply()` 跑过），**不断言 HTTP**，也 **不** 要求默认 3080 空闲。活 tools 表仍然不在 verify 范围内。
...
- 把 headless 当成「已经在监督的 web 宿主」的热切换；若 `dshx already supervises`，先 [stop / restart](restart-outside.md)
