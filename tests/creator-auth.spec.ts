...
// @ts-expect-error auth.js ships no type declarations
...
test('official startup input is origin-bound and diagnostics never echo it', () => {
  ...
  for (const invalid of [url.replace('43127','3080'), url+'&token=again', url+'#fragment', url.replace('/?', '/other?')]) {
    assert.throws(() => privateStartupUrl(() => invalid, 43127), error => error instanceof Error && /WEB_AUTH_/.test(error.message) && !error.message.includes(secret))
  }
  assert.throws(() => privateStartupUrl(() => { throw new Error(url) }, 43127), error => !(error instanceof Error) || !error.message.includes(secret))
